/**********************************************************************************************************************
 * \file Cpu0_Main.c
 * \copyright Copyright (C) Infineon Technologies AG 2019
 *
 * Use of this file is subject to the terms of use agreed between (i) you or the company in which ordinary course of
 * business you are acting and (ii) Infineon Technologies AG or its licensees. If and as long as no such terms of use
 * are agreed, use of this file is subject to following:
 *
 * Boost Software License - Version 1.0 - August 17th, 2003
 *
 * Permission is hereby granted, free of charge, to any person or organization obtaining a copy of the software and
 * accompanying documentation covered by this license (the "Software") to use, reproduce, display, distribute, execute,
 * and transmit the Software, and to prepare derivative works of the Software, and to permit third-parties to whom the
 * Software is furnished to do so, all subject to the following:
 *
 * The copyright notices in the Software and this entire statement, including the above license grant, this restriction
 * and the following disclaimer, must be included in all copies of the Software, in whole or in part, and all
 * derivative works of the Software, unless such copies or derivative works are solely in the form of
 * machine-executable object code generated by a source language processor.
 *
 * THE SOFTWARE IS PROVIDED "AS IS", WITHOUT WARRANTY OF ANY KIND, EXPRESS OR IMPLIED, INCLUDING BUT NOT LIMITED TO THE
 * WARRANTIES OF MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE, TITLE AND NON-INFRINGEMENT. IN NO EVENT SHALL THE
 * COPYRIGHT HOLDERS OR ANYONE DISTRIBUTING THE SOFTWARE BE LIABLE FOR ANY DAMAGES OR OTHER LIABILITY, WHETHER IN
 * CONTRACT, TORT OR OTHERWISE, ARISING FROM, OUT OF OR IN CONNECTION WITH THE SOFTWARE OR THE USE OR OTHER DEALINGS
 * IN THE SOFTWARE.
 *********************************************************************************************************************/
#include "Ifx_Types.h"
#include "IfxCpu.h"
#include <ifxCpu_Irq.h>
#include "IfxScuWdt.h"
#include "SmartCar_Uart.h"
#include "SmartCar_Upload.h"
#include "SmartCar_Oled.h"
#include "SmartCar_Pwm.h"
#include "SmartCar_MPU.h"
#include "SmartCar_MT9V034.h"
#include "SmartCar_Systick.h"
#include "SmartCar_GPIO.h"
#include "SmartCar_PIT.h"
#include "SmartCar_Encoder.h"
#include "SmartCar_ADC.h"
#include "common.h"
#include "image.h"
#include "menu.h"
#include "beacon_control.h"
#include "testsome.h"
#include "VA_get.h"
#include "EMIT_Pitmgr.h"
#include "EMIT_List.h"
#include "SmartCar_Assert.h"
#include <adrc.h>
#include "tfmini.h"


#pragma section all "cpu0_dsram"
//IfxCpu_syncEvent g_cpuSyncEvent;
mpu_t this_mpu;
float time;
float myvar[20] = {0};
extern int16 leftFTM;
extern int16 rightFTM;
/*电压电流采集值*/
extern float battery;
extern float Icharge;
extern float battery_last;
extern float battery_prelast;
extern float powerduty;
/**/


/*extern专区*/
extern int mode_flag;
extern int error_simu;
extern float servo_pulse;
extern int light_flag;
extern int aver_x;
extern int simu_y;
extern int motordutyset;
extern float motor_rigdevn;
extern float motor_lefdevn;
extern int side_flag;
extern int servoinit;
extern int flagservo;
extern float Pcharge;
extern float powererror;

extern float MIcharge16;
extern float MIcharge17;

extern float power;
extern float setpower;
extern float setpower1;
extern uint32 powerduty1;
extern int countflag;
extern float chargeKp;
extern float chargeKi;
extern float chargeKd;
extern float chargeek;
extern float chargeekL;
extern float chargedek;
extern int lightsensor[2];
extern int speedmark;
/**/


extern uint32_t motor_lefdevnU;
extern uint32_t motor_rigdevnU;
extern uint32_t N_motor_lefdevnU,N_motor_rigdevnU;
extern uint32 Flefdevn;
extern uint32 Frigdevn;
extern uint32 NFlefdevn,NFrigdevn;
extern uint8 connectnum;
extern int areaA_time;
extern uint32_t servofinal;
extern int servodirection;
extern float lighty;
extern int aver_y;
extern float lightx,lights,lightc;
extern int servoduty;
extern float Rdesiredspeed;
extern float Ldesiredspeed;
extern int light_square;
extern int squarelimit;
extern int distance;

int status = 0;
int gogoflag = 0;
/*oled灯和图像的切换*/
int mode = 0;
int timeget = 0;
float acc[3] = {0};
float gyro[3] = {0};
float gyronow[3] = {0};
extern float angle_x,angle_y,angle_z;

extern int freshposi;
extern int freshname;

pitmgr_handle_t* ctrl_motorCtrlHandle = NULL;
pitmgr_handle_t* ctrl_servoCtrlHandle = NULL;
pitmgr_handle_t* ctrl_chargeCtrlHandle = NULL;

#define CTRL_MOTOR_CTRL_MS 5
#define CTRL_SERVO_CTRL_MS 3
#define CTRL_CHARGE_CTRL_MS 5

float L_adc[8] = {0};
extern float Ldirs ;
extern float Rdirs ;
extern float Rb;
extern float Rkp;
extern float Rkd;
extern float Rw0;
extern pidCtrl_t  servo_pid;


extern adrc_t Lctrl;
extern adrc_t Rctrl;
extern int d;

int a = 0;
int servodutydd;
int aa = 0;
extern float lightdis;
int position2[2] = {0};
int lightx2 = 0;
int lighty2 = 0;
int area2 = 0;
int dis2 = 0;

int tfmini[4] = {0};

extern uint8 Dist_L;
extern uint8 Dist_H;
extern int Dist;

extern uint8 Strength_L;
extern uint8 Strength_H;
extern int Strength;
extern int tempbuff2;
extern uint8 statustf;
extern int camera_last,camera_flag;
extern int back_flag;

extern float Angle_X_Final;
extern float Angle_Y_Final;
extern float Angle_x_temp;
extern float Angle_y_temp;
extern int servoduty;
extern int go_flag;    //进入跑车程序标志位


extern int flag1;
int switch0=1;//第4个
int switch1=1;//第3个
int switch2=1;//第2个（拨码坏）
int switch3=1;//第1个

 int core0_main(void)
{
    IfxCpu_disableInterrupts();

    get_clk();
    /* !!WATCHDOG0 AND SAFETY WATCHDOG ARE DISABLED HERE!!
     * Enable the watchdogs and service them periodically if it is required
     */
    IfxScuWdt_disableCpuWatchdog(IfxScuWdt_getCpuWatchdogPassword());
    IfxScuWdt_disableSafetyWatchdog(IfxScuWdt_getSafetyWatchdogPassword());

    /* Wait for CPU sync event */
    IfxCpu_emitEvent(&g_cpuSyncEvent);
    IfxCpu_waitEvent(&g_cpuSyncEvent, 1);

    /****初始化外设****/
    /**/

    /*总钻风初始化*/
    SmartCar_MT9V034_Init();
//
    /*初始化mpu*/
    //ICM20602
//    SmartCar_MPU_Set_DefaultConfig(&this_mpu);
//    SmartCar_MPU_Init2(&this_mpu);
   //mpu6050
//    SmartCar_MPU_Set_DefaultConfig(&this_mpu);
//    SmartCar_MPU_Init2(&this_mpu);
  // SmartCar_GyroOffset(this_mpu);
    /**/
    /**/
    /*定时中断初始化*/
    /*电机*/
    Pit_Init(CCU6_0,PIT_CH0,5*1000);
//    Pit_Enable_Interrupt(CCU6_0,PIT_CH0);
//    Pit_Start(CCU6_0,PIT_CH0);
    //控制
      Pit_Init(CCU6_1,PIT_CH0,2*1000);
//    /*舵机*/
    Pit_Init(CCU6_1,PIT_CH1,3*1000);
//    Pit_Init(CCU6_1,PIT_CH1,20*1000);
//
//      Pit_Init(CCU6_0,PIT_CH1,1*1000);
////
//    PITMGR_Init();
//    ctrl_motorCtrlHandle = PITMGR_HandleInsert(CTRL_MOTOR_CTRL_MS,2U,Motor_control,pitmgr_pptEnable);
//    assert(ctrl_motorCtrlHandle);
//    ctrl_servoCtrlHandle = PITMGR_HandleInsert(CTRL_SERVO_CTRL_MS,0U,Servo_control,pitmgr_pptEnable);
//    assert(ctrl_servoCtrlHandle);
//    ctrl_chargeCtrlHandle = PITMGR_HandleInsert(CTRL_CHARGE_CTRL_MS,3U,BIget,pitmgr_pptEnable);
//    assert(ctrl_chargeCtrlHandle);
    /*PWM初始化*/
  Servo_motorinit();
//    Ctrlinit();
//    Ctrltinit();
//    SmartCar_Gtm_Pwm_Init(&IfxGtm_ATOM0_6_TOUT6_P02_6_OUT,20000,0);
    /**/

//    /*编码器初始化*/
    Encoder_init();
//
//    /*串口初始化*/
    SmartCar_Uart_Init(IfxAsclin0_TX_P14_0_OUT,IfxAsclin0_RXA_P14_1_IN,921600,0);
    SmartCar_Uart_Init(IfxAsclin3_TX_P21_7_OUT,IfxAsclin3_RXF_P21_6_IN,921600,3);

//    TFmini_init();

    /**/

    /*GPIO初始化*/
    /*菜单用GPIO初始化*/
    Menu_gpioinit();
    GPIO_Init(P20,9,PUSHPULL,0);

    //蜂鸣器
//
    /*ADC初始化*/
    ADC_Init(ADC_0,ADC0_CH11_A11);
    ADC_Init(ADC_0,ADC0_CH8_A8);

    /*电流环(3,4对应)*/
 //   ADC_Init(ADC_1,ADC1_CH0_A16);//����

    /*电流环(2,5对应)*/
 //   ADC_Init(ADC_1,ADC1_CH1_A17);//��ѹ


    ADC_Init(ADC_0,ADC0_CH2_A2);//����
    ADC_Init(ADC_0,ADC0_CH3_A3);//��ѹ

    //按键中断
//    Eru_Init(CH4_P33_7,FALLING);
    GPIO_Init(P15,8,NO_PULL,1);
    GPIO_Init(P15,4,NO_PULL,1);
    GPIO_Init(P33,7,NO_PULL,1);
    GPIO_Init(P15,7,NO_PULL,1);
    GPIO_Init(P15,6,NO_PULL,1);

    /**/
    /*OLED初始化*/
    SmartCar_Oled_Init();
//    /*初始化菜单*/
    SmartCar_OLED_Fill(0);
    MENU_Tree();
//    /*显示首页面*/

    MENU_Start();
//    MENU_storage();
    MENU_Read();
//    GPIO_Init(P20,2,PUSHPULL,0);

    IfxCpu_enableInterrupts();

    while(TRUE)
    {
//        while(!mt9v034_finish_flag){}
//            mt9v034_finish_flag = 0;
//        Systick_Start(STM0);
//        THRE();
//        image_main();
//        time = GetTime_ms(STM0);
//        myvar[0] = time;
//        /*测输入电压电流*/
//        GetVA();
//        myvar[1] = CV_IN;
//        myvar[2] = CA_IN; //电流
//        Imudata_get();
//        Imudata_get();

//        myvar[3] = side_flag;
//        myvar[4] = light_flag;
//        myvar[5] = gyro[2];
////        myvar[6] = ;
////        myvar[7] = battery;
////
////        myvar[8] = Icharge;

//        MIcharge_Get1();
//        MIcharge_Get2();
//        Imudataprocess();//获取车身当前与水平面夹角
//        if(mode == 0){
//        myvar[0] = mode_flag;
//        myvar[1] = rightFTM;
//        myvar[2] = motor_rigdevn;
////        myvar[3] = Rctrl.u;
////        myvar[4] = Rctrl.kp;
////        myvar[5] = Rctrl.kd;
////        myvar[6] = Rctrl.w0;
////        myvar[7] = Rctrl.b;
////        myvar[8] = Rkp;
//        SmartCar_VarUpload(&myvar,9);
//        MIcharge_Get1();
//        MIcharge_Get2();
//        Systick_Start(STM0);
//        Prepare_Data();
//        Imu_Update(gx,gy,gz,ax,ay,az);
//        time = GetTime_us(STM0);
//        leftFTM = SmartCar_Encoder_Get(GPT12_T4);
//        rightFTM = SmartCar_Encoder_Get(GPT12_T6);
        switch1 = GPIO_Read(P15,4);
        switch2 = GPIO_Read(P15,6);
//        switch0 = GPIO_Read(P15,8);
//        switch3 = GPIO_Read(P15,7);
////        MENU_fresh();
        if(1 == switch1&&1 == switch2)
        {
            MENU_Make();
            //SmartCar_VarUpload(myvar,9);
//            MENU_fresh();
        }
        else if(0==switch1&&1 == switch2)
        {
            MENU_fresh();
            //MENU_Make();
        }
        else if(0==switch1&&0 == switch2)
        {
            SmartCar_Show_IMG(&IMG[0][0],96,150);
        }
//        /**/
//        /*按键操作*/
//        else if(mode == 1){
           // SmartCar_Show_IMG(&IMG,120,188);
//        }
//        else if(mode == 2)
//        {
//            SmartCar_VarUpload(myvar,9);
//        }
//        else if(mode == 3){}
//
//        if(battery>=9&&battery_last>=9&&battery_prelast>=9&&timeget == 500)
//        {
//            gogoflag = 1;
//        }

        /*采图*/
//      SmartCar_ImgUpload((uint8*)IMG,120,188);  //采图用函数
//      SmartCar_ImgUpload((uint8*)mt9v034_image,96,150);  //采图用函数
       /**/
    }
}
//oled刷新
 MENU_freshadd()
 {
     MENU_freshfloat("battery",battery);
//     MENU_freshint("gogoflag",gogoflag);
//     MENU_freshint("goflag",go_flag);
//     MENU_freshfloat("l",Ldesiredspeed);
//     MENU_freshfloat("r",Rdesiredspeed);
//      MENU_freshint("1",leftFTM);
//      MENU_freshint("2",rightFTM);
//     MENU_freshfloat("lefdevn",Flefdevn);
//     MENU_freshfloat("rigdevn",Frigdevn);
//     MENU_freshfloat("Nlefdevn",NFlefdevn);
//     MENU_freshfloat("Nrigdevn",NFrigdevn);
//     MENU_freshfloat("battery",battery);
     MENU_freshfloat("xx",lightx);
     MENU_freshfloat("yy",lighty);
     MENU_freshfloat("dis1",lightdis);

     MENU_freshint("2x",lightx2);
     MENU_freshint("2y",lighty2);
//     MENU_freshint("dis2",dis2);
     MENU_freshint("camflag",camera_flag);
     MENU_freshfloat("back_flag",back_flag);
//     MENU_freshfloat("Angle_x_temp",Angle_x_temp);
//     MENU_freshfloat("Angle_y_temp",Angle_y_temp);
//     MENU_freshfloat("Angle_err",Angle_err);
//     MENU_freshfloat("Q_bias",Q_bias);
//    MENU_freshfloat("K_0",K_0);
//     MENU_freshfloat("gx",gyronow[0]);
//     MENU_freshfloat("gy",gyronow[1]);
//     MENU_freshfloat("angle_z",angle_z);
//     MENU_freshfloat("ax",ay);
//     MENU_freshint("ax",az);
//     MENU_freshfloat("Ldesiredspeed",Ldesiredspeed);
//     MENU_freshfloat("distance",distance);
//     MENU_freshfloat("aver_x",aver_x);
//          MENU_freshfloat("L_adc[4]",L_adc[4]);
//          MENU_freshfloat("L_adc[5]",L_adc[5]);
//          MENU_freshfloat("L_adc[6]",L_adc[6]);
//          MENU_freshfloat("L_adc[7]",L_adc[7]);
 }


/*定时器中断*/
/*20ms*/
IFX_INTERRUPT(cc60_pit_ch0_isr, 0, CCU6_0_CH0_ISR_PRIORITY)
{
    enableInterrupts();//开启中断嵌套
    PIT_CLEAR_FLAG(CCU6_0, PIT_CH0);
//    PITMGR_Isr();
    Motor_control();
//    Motorctrl();
}
///*5ms*/
IFX_INTERRUPT(cc61_pit_ch1_isr, 0, CCU6_1_CH1_ISR_PRIORITY)
{
    enableInterrupts();//开启中断嵌套
    Servo_control();
//    SmartCar_Gtm_Pwm_Init(&IfxGtm_ATOM1_1_TOUT31_P33_9_OUT,333,servoduty);

//    SmartCar_Gtm_Pwm_Setduty(&IfxGtm_ATOM1_1_TOUT31_P33_9_OUT,servodutydd);
    PIT_CLEAR_FLAG(CCU6_1, PIT_CH1);
}
///*5ms*/
IFX_INTERRUPT(cc61_pit_ch0_isr, 0, CCU6_1_CH0_ISR_PRIORITY)
{
    enableInterrupts();//开启中断嵌套
    PIT_CLEAR_FLAG(CCU6_1, PIT_CH0);
    FINAL_CONTROL();
    BIget();

//    if(a <= 6)
//    {
//        aa = aa+15;
//        a++;
//    }
//    if(a>6)
//    {
//        a = 0;
//        aa = 0;
//    }
}

IFX_INTERRUPT(cc60_pit_ch1_isr, 0, CCU6_0_CH1_ISR_PRIORITY)
{
    enableInterrupts();//开启中断嵌套
    PIT_CLEAR_FLAG(CCU6_0, PIT_CH1);
}
/**/

MENU_fresh()
{
    Delay_ms(STM0,100);
    freshposi=0;
    MENU_freshadd();
    freshname=1;

}



#pragma section all restore
