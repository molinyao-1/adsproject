...
void List_Constructor(list_struct_t *p, unsigned int SizeOfData) {
    assert(p);
    assert(SizeOfData);
    memset(p, 0, sizeof(list_struct_t));
    p->SizeOfData = SizeOfData;
    p->size = 0;
    //����head��tail
    p->head = (list_node_t*)malloc(sizeof(list_node_t));
    assert(p->head);
    p->tail = p->head;
    //����
    p->head->next = NULL;
...
uint32_t List_GetSize(list_struct_t *p) { return p->size; }
...
list_node_t* List_Create(list_struct_t* p, list_node_t* pos)
{
    ...
    //assert(src);
    list_node_t* newl;
    newl = (list_node_t*)malloc(sizeof(list_node_t));
    assert(newl);
    newl->data = malloc(p->SizeOfData);
    assert(newl->data);
    newl->prev = pos->prev;
    newl->next = pos;
    if (newl->prev != NULL)
...
list_node_t* List_Insert(list_struct_t *p, list_node_t *pos, void *src) {
    assert(src);
...
void* List_GetFront(list_struct_t *p) { return p->head->data; }

void* List_GetBack(list_struct_t *p) { return p->tail->prev->data; }

list_node_t* List_ItBegin(list_struct_t *p) { return p->head; }

list_node_t* List_ItEnd(list_struct_t *p) { return p->tail; }
