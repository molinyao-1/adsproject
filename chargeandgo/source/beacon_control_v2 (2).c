...
void CAMERA_JUDGE()
{
    ...
    light_squarerf = 0.25*light_square+0.75*light_squarerf;
    light_square = light_squarerf;
}
...
//        if(lightsensor[0] == 0||lightsensor[1] == 0)
//        {
//            R_time++;
//        }
//        if(R_time>=1)
...
{
    if(0==mode_flag||3==mode_flag||(mode_flag == 1&&(light_square<squarelimit)))
    {
    servodirection = 0;
    }
    if((light_square>squarelimit))  //结束近灯处于灭灯旋转状态
    {
...
void Motor_control2()
{
    ...
    if(mode_flag == 0||(light_square<squarelimit)||mode_flag == 3) // 除灭灯状态，其余状态计算电机的减速线
    {
        slowline = (int)(120-(leftFTM + rightFTM)*correctmenu);
        if(slowline<25)
            LOW = 25;
...
//        }
    ...
    else if(mode_flag == 2||(light_square>squarelimit))//看不到灯
    {
        Ldesiredspeed = slowspeedwant;
        Rdesiredspeed = slowspeedwant;
        if(servodirection == 1) //左转
