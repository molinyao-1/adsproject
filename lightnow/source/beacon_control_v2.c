...
void ERROR()
{
    ...
    if((mode_flag == 3)|| (mode_flag == 0 && go_flag == 1)/*进入追灯状态且跑车标志位为1*//* || light_change_flag == 1/*可切换切灯方向为1*/)
    {  //确定切左还是切右！！！！！！！！
...
void STOP_JUDGE()
{
    if(areaA_time == 0)
        ...
        roll_time ++;
    if(roll_time > 2000)  //设为0则会长时间无灯停车
    {
        go_flag = 0;
        roll_time = 2000;
    }
...
//    angle_y += (gyro[1]-gyrooffset1)*dt;
    if((abs(servo_pulse)>30)||(servodirection == 1)||(servodirection == -1)&&(go_flag))
